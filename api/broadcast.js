...
async function getFirebaseToken() {
  try {
    ...
    const payload = btoa(JSON.stringify({
      ...
      iat: now, exp: now + 3600, scope: "https://www.googleapis.com/auth/firebase.database"
    } ));
    const key = await crypto.subtle.importKey("pkcs8", pemToBuffer(PRIVATE_KEY), { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["sign"]);
...
async function sendMessage(chatId, text, button = null) {
  // أزلنا parse_mode لتجنب أخطاء التنسيق
  ...
  const data = await res.json();
  if (!data.ok) throw new Error(data.description); // رمي الخطأ إذا رفض تيليجرام الرسالة
}
...
export default async function handler(req, res) {
  ...
  if (key !== ADMIN_KEY) return res.status(401).json({ error: "المفتاح السري غير صحيح" });
  ...
  try {
    ...
    if (!chatIdsObj) return res.json({ success: true, sent: 0, message: "لا يوجد مستخدمين بعد" });
    ...
    for (let i = 0; i < chatIds.length; i++) {
      try {
        ...
        failed++; 
        lastError = e.message; // حفظ سبب الرفض
      }
      if (i % 20 === 19) await new Promise(r => setTimeout(r, 1000));
    }

    if (failed > 0 && sent === 0) {
      return res.status(400).json({ error: `رفض تيليجرام الرسالة: ${lastError}` });
    }
